...
def main():
    ...
    if not os.path.exists(os.path.dirname(strname)):
        os.makedirs(os.path.dirname(strname))
    ...
    cell_ids = gexpr_feature.index.tolist()
    cell_id_save_path = os.path.join(args.save_path, args.task_name + '_' + args.ckpt_name + "_cell_ids.txt")
    with open(cell_id_save_path, 'w') as f:
        f.write("\n".join(cell_ids))
